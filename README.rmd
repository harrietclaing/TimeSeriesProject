...
The methodology that I applied in order to replicate the study, was to first transform all of the quarterly series in logarithms except for the interest rate. As in the paper, we run Augmented Dickey Fuller tests on all the time series variables. Findings were that Interest Rates are stationary, inflation before transformed is non-stationary, integrated order 1, and after transformed is stationary, real GDP is integrated in order 1, monetary aggregate is integrated order 1 before and after it is transformed, exchange rates and world oil price also integrated first order. Whereas the paper found that interest rates, real GDP, exchange rates and the world oil price was integrated of order 1, whereas the monetary aggregate and inflation were found to be integrated of order 2. Despite the differences in integration order, I then followed the paper by transforming these variables that were integrated of order 2 by using the real monetary aggregate (by taking the difference between the logarithm of monetary aggregate and the logarithm of inflation) and then using the change in logarithm inflation instead of logarithm of inflation.

Intially found that matrix was computationally singular, therefore, needed to change matrix so it could be invertible. Let us check for multicollinearity and found all variables were correlated and statistically significant. Tried to test correlation if did not transform monetary aggregate and inflation as paper did, but still highly correlated.

Perhaps, because in the paper they used predominantly seasonally-adjusted data but due to constraints on availability of data, for this replication I used a combination of seasonally-adjusted and not seasonally-adjusted.

Could only overcome this error by detrending GDP and monetary aggregate. Therefore, our VAR system includes all the endogenous variables of interest rate, inflation, exchange rate, monetary aggregate and real GDP. In the following section, the exogenous variable of world oil prices and the time trend will be accounted for in the test for cointegration.
...
autoplot
```
We then construct our VAR model by creating a matrix which includes the time series variables included in Figure 1. The VAR model was found to have 9 lags (10 - 1) when we accounted for a time trend, as is done in the paper.
...
# to get a matrix where each column


#get each cointegrating vector alone
cointegrating_vectors

InterestRateCV <- cointegrating_vectors[,1]
InterestRateCV

InflationCV <- cointegrating_vectors[,2]
InflationCV

RealGDPCV <- cointegrating_vectors[,3]

MoneyCV <- cointegrating_vectors[,4]

ExchangeRateCV <- cointegrating_vectors[,5]

OilPriceCV <- cointegrating_vectors[,6]
```


```{r}
#remove the trend line from the matrix so that it is 6x6
cointegrating_matrix <- cointegrating_matrix[-7,-7]
    
colnames(cointegrating_matrix)<-NULL
rownames(cointegrating_matrix) <- NULL
```
...
Check if the cointegrating relationships are trending and not random walks, by first testing for stationarity.
```{r}

adf.IntRateCV <- ur.df(InterestRateCV, type="none", selectlags = c("AIC"))
summary(adf.IntRateCV)
#stationary

adf.InflationCV <- ur.df(InflationCV, type="none", selectlags = c("AIC"))
summary(adf.InflationCV)
#non stationary

adf.RealGDPCV <- ur.df(RealGDPCV, type="none", selectlags = c("AIC"))
summary(adf.RealGDPCV)
#stationary

adf.MoneyCV <- ur.df(MoneyCV, type="none", selectlags = c("AIC"))
summary(adf.MoneyCV)
#stationary

adf.ExchangeCV <- ur.df(ExchangeRateCV, type="none", selectlags = c("AIC"))
summary(adf.ExchangeCV)
#non stationary

adf.OilPriceCV <- ur.df(OilPriceCV, type="none", selectlags = c("AIC"))
summary(adf.OilPriceCV)
```
...
```{r}
adf.MonetaryAggregate <- ur.df(MonetaryAggregateM1_US$MonetaryAggregateM1, type = "none", selectlags = c("AIC"))

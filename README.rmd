...
World oil price: could only find from 1990
```{r}
...
# Transformations of Monetary Aggregate and Inflation
...
```{r}
dfMonetaryAggregate_Inflation <- data.frame(DATE=logMonetaryAggregateM1_US$DATE, logInflation=logInflation_US$logInflation, logMonetaryAggregateM1= logMonetaryAggregateM1_US$logMonetaryAggregateM1)
dfMonetaryAggregate_Inflation
```
...
Then, change in price level??? we need the previous value to add as a change
```{r}
TransformedInflation_US <- mutate(dfMonetaryAggregate_Inflation, TransformedInflation = dfMonetaryAggregate_Inflation$logInflation - lag(dfMonetaryAggregate_Inflation$logInflation, default=first(dfMonetaryAggregate_Inflation$logInflation)))
TransformedInflation_US
```
...
```{r}
df_US <- data.frame(DATE=logExchangeRates_US$DATE, InterestRate=InterestRates_US$InterestRate, Inflation= TransformedInflation_US$TransformedInflation, RealGDP=logRealGDP_US$logRealGDP, MonetaryAggregate=TransformedMonetaryAggregateM1_US$TransformedMonetaryAggregateM1, ExchangeRates=logExchangeRates_US$logExchangeRates, WorldOilPrice=WorldOilPrice_dollars$WorldOilPrice)
...
```{r}
?ca.jo
jotest <- ca.jo(df_US, type="trace", K=2, ecdet="none", spec="longrun")
summary(jotest)
```
